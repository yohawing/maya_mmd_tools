...
`mmd_tools/tools/maya_bake_oracle_dumper.py` は、Maya シーンへ PMX/VMD を読み込んで
runtime bake を実行した後、`cmds.currentTime()` で各フレームを評価し、最終的な
joint worldMatrix と blendShape weight を JSONL に出力します。
以後の数値検証では、mmd-anim runtime と MMD 実機ダンプの残差ではなく、
この Maya bake 後の結果を Oracle として扱います。

使用例:

```bash
mayapy -m mmd_tools.tools.maya_bake_oracle_dumper \
  --manifest F:\Develop\MMDDev\GoldenOracle\manifests\motion-numeric.json \
  --case tda-miku-melancholy-night \
  --sample-frame-offset 1
```

Maya bake Oracle 生成では、runtime が解決した最終姿勢だけを保存するため、
PMX インポート時に内部で `setup_rig=False` と `setup_bone_orientation=False` を指定しています。
これにより、RigConverter の付与 constraint だけでなく、PMX ローカル軸/軸固定由来の `jointOrient` も作らず、
Maya 側リグと runtime bake の二重評価を避けます。

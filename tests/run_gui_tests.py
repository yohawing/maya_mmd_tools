...
def validate_attach_manifest(cases, project_root=None):
    """Require explicit opt-in for the fixed, scene-read-only smoke set."""
    ...
    rejected = [
        f"{case.get('id', '<unknown>')} ({case.get('test_path')!r}, {case.get('test_filter')!r})"
        for case in cases
        if (case.get("test_path"), case.get("test_filter")) not in ATTACH_SAFE_CASES
    ]
    if rejected:
        raise ValueError(
            "--attach-existing allows only the fixed read-only smoke cases; "
            "rejected case(s): " + ", ".join(rejected)
        )
    return cases

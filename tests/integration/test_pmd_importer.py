...
class TestPmdImporter(MayaTestBase):
    ...
    def test_import_pmd_with_scale_factor(self):
        """スケールファクターを適用したインポートテスト"""
        try:
            pmd_file = self.fixture_provider.get_pmd_file()
        except FileNotFoundError:
            self.skipTest("テスト用PMDファイルが見つかりません")
        ...
        original_scale = settings.get("import.general.scale_factor", 1.0)
        settings.set("import.general.scale_factor", 10.0)
        ...
        try:
            ...
            meshes = cmds.ls(type='mesh')
            if meshes:
                # 親トランスフォームノードのスケールを確認
                for mesh in meshes:
                    transform = cmds.listRelatives(mesh, parent=True)[0]
                    scale = cmds.getAttr(f"{transform}.scale")[0]
                    # スケールが適用されていることを確認（実装による）
        finally:
            # スケールファクターを元に戻す
            settings.set("import.general.scale_factor", original_scale)
    ...
    def test_import_pmd_with_materials(self):
        ...
        # マテリアルが作成されたことを確認
        materials = cmds.ls(type='lambert') + cmds.ls(type='phong') + cmds.ls(type='blinn')
        # デフォルトマテリアルを除外
    ...
    def test_import_pmd_bone_hierarchy(self):
        ...
        # ジョイントが作成されたことを確認
        joints = cmds.ls(type='joint')
        self.assertEqual(len(joints), len(parser.bones), 

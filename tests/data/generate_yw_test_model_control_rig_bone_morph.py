...
legacy PMX writer.  Explicit standard, semi-standard, custom, and mixed-target
BoneMorph cases and their VMD weight keys are added from semantic source JSON;
no binary hand editing is permitted.
"""
...
def _morph_specs(source: Mapping[str, Any]) -> tuple[Mapping[str, Any], ...]:
    specs = source.get("bone_morphs")
    if not isinstance(specs, list) or not specs:
        raise ValueError("bone_morphs source must be a non-empty list")
    result = []
    names = set()
    for spec in specs:
        if not isinstance(spec, Mapping):
            raise ValueError("each bone_morphs entry must be an object")
        name = str(spec.get("name", ""))
        targets = spec.get("targets")
        frames = spec.get("frames")
        if not name or name in names or not isinstance(targets, list) or not targets:
            raise ValueError("bone morph names must be unique and targets must be non-empty")
        if [int(entry.get("frame", -1)) for entry in frames or () if isinstance(entry, Mapping)] != [0, 10, 20]:
            raise ValueError("bone morph frames must be exactly [0, 10, 20]")
        names.add(name)
        result.append(spec)
    return tuple(result)
...
def generate_model(
    ...
    """Generate the PMX with the declared BoneMorph coverage matrix."""

    source = _source_data(source_path)
    specs = _morph_specs(source)
    ...
    for spec in specs:
        ...
        for target in spec["targets"]:
            if not isinstance(target, Mapping):
                raise ValueError("BoneMorph target must be an object")
            bone_name = str(target.get("bone_name", ""))
            expected_category = str(target.get("category", ""))
            if _bone_category(bone_name) != expected_category:
                raise ValueError(
                    f"BoneMorph category mismatch for {bone_name!r}: {expected_category!r}"
                )
            bone_indices = [
                index for index, bone in enumerate(pmx.bones) if bone.name == bone_name
            ]
            if len(bone_indices) != 1:
                raise ValueError(
                    f"BoneMorph target must resolve exactly one PMX bone: {bone_name!r}"
                )
            translation = tuple(float(value) for value in target["translation"])
            rotation = tuple(
                float(value)
                for value in target.get("rotation", (0.0, 0.0, 0.0, 1.0))
            )

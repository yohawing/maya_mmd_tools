...
class TestBakeModeBehavior(unittest.TestCase):
    ...
    def test_model_import_builds_rig_even_when_bake_mode_enabled(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("mmd_tools.io.pmx_importer.cmds.group", return_value="model_root"))
            stack.enter_context(patch("mmd_tools.io.pmx_importer.cmds.select"))
            stack.enter_context(patch("mmd_tools.io.pmx_importer.cmds.refresh"))
            stack.enter_context(patch("mmd_tools.io.pmx_importer.maya_utils.set_custom_attributes"))
            stack.enter_context(patch("mmd_tools.io.pmx_importer.maya_utils.setup_mmd_color_management"))
            stack.enter_context(patch("mmd_tools.io.pmx_importer.maya_utils.setup_mmd_transparency"))
            stack.enter_context(patch("mmd_tools.io.pmx_importer.sync_dx11_generated_uniforms", return_value=0))

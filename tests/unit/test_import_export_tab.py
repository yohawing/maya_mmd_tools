...
class TestExportTabNavigationAndActions(unittest.TestCase):
    ...
    def test_supported_languages_define_model_animation_and_format_actions(self):
        ...
        expected = {
            "ja": ("モデル", "アニメーション", "ポーズ", "モデルを書き出す", "アニメーションを書き出し", "ポーズを書き出し"),
            "en": ("Model", "Animation", "Pose", "Export Model", "Export Animation", "Export Pose"),
            "zh_cn": ("模型", "动画", "姿势", "导出模型", "导出动画", "导出姿势"),
            "zh_tw": ("模型", "動畫", "姿勢", "匯出模型", "匯出動畫", "匯出姿勢"),
        }
        for language, values in expected.items():
            ...
            self.assertEqual(translations["tabs"]["export_pose"], values[2])
            self.assertEqual(translations["buttons"]["export_pmx"], values[3])
            self.assertEqual(translations["buttons"]["export_vmd"], values[4])
            self.assertEqual(translations["buttons"]["export_vpd"], values[5])

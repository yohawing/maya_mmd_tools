...
from mmd_tools.core import maya_attribute_utils, maya_material_utils  # noqa: E402
...
class TestMayaMaterialUtilsTextureProvenance(unittest.TestCase):
    ...
    def test_get_mmd_original_texture_path_decodes_legacy_base64_when_path_like(self):
        ...
        with patch.object(maya_attribute_utils, "get_attribute", return_value=encoded):
            self.assertEqual(maya_material_utils.get_mmd_original_texture_path("file1"), "textures/髪.png")

    def test_get_mmd_original_texture_path_keeps_plain_text(self):
        with patch.object(maya_attribute_utils, "get_attribute", return_value="textures/髪.png"):
            self.assertEqual(maya_material_utils.get_mmd_original_texture_path("file1"), "textures/髪.png")

    def test_shared_toon_file_node_is_not_classified_for_path_resolve(self):
        with patch.object(maya_attribute_utils, "get_attribute", return_value="shared_toon"):
            self.assertIsNone(maya_material_utils.classify_mmd_texture_file_node("file1"))

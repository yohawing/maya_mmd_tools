...
from mmd_tools.core import (
    ...
    maya_attribute_utils,
    maya_material_utils,
    maya_mesh_utils,
    maya_rig_utils,
    maya_scene_utils,
    maya_transform_utils,
    utils,
)
...
class TestMayaUtils(MayaTestBase):
    def test_create_mesh_with_uvs(self):
        ...
        mesh_transform = maya_mesh_utils.create_mesh_with_uvs(name, vertices, face_counts, face_connects, uvs, face_uv_connects)
    ...
    def test_create_material(self):
        ...
        shader_node = maya_material_utils.create_material(material_name, color)
    ...
    def test_assign_material(self):
        ...
        # Create a material
        shader_node = maya_material_utils.create_material(material_name, (1, 0, 0, 1))

        # Assign the material
        maya_material_utils.assign_material(mesh_name, shader_node)
    ...
    def test_set_attribute(self):
        ...
        maya_attribute_utils.set_attribute(mesh_name, "translateX", 5.0, "double")
        self.assertAlmostEqual(cmds.getAttr(mesh_name + ".translateX"), 5.0, places=5)

        maya_attribute_utils.set_attribute(mesh_name, "translateY", -3.0, "double")
        self.assertAlmostEqual(cmds.getAttr(mesh_name + ".translateY"), -3.0, places=5)

        maya_attribute_utils.set_attribute(mesh_name, "scaleX", 2.0, "double")
        self.assertAlmostEqual(cmds.getAttr(mesh_name + ".scaleX"), 2.0, places=5)

        # Test 2: 複数値を持つ既存アトリビュート
        maya_attribute_utils.set_attribute(mesh_name, "translate", [1.0, 2.0, 3.0], "double3")
        ...
        maya_attribute_utils.set_attribute(mesh_name, "customIntAttr", 42, "long")
        self.assertEqual(cmds.getAttr(mesh_name + ".customIntAttr"), 42)

        # カスタムアトリビュートの更新
        maya_attribute_utils.set_attribute(mesh_name, "customIntAttr", 100, "long")
        self.assertEqual(cmds.getAttr(mesh_name + ".customIntAttr"), 100)

        # Test 5: visibility属性（bool）
        maya_attribute_utils.set_attribute(mesh_name, "visibility", False, "bool")
        self.assertEqual(cmds.getAttr(mesh_name + ".visibility"), False)
        maya_attribute_utils.set_attribute(mesh_name, "visibility", True, "bool")
        self.assertEqual(cmds.getAttr(mesh_name + ".visibility"), True)

        # Test 6: エラーケース - 存在しないオブジェクト
        # set_attributeは例外を再発生させないため、エラー時も正常に処理される
        # 値が設定されないことを確認
        maya_attribute_utils.set_attribute("non_existent_object", "translateX", 1.0, "double")
        # ログにエラーが出力されることを期待（実際のテストではログ出力の確認は省略）

        # Test 7: エラーケース - 存在しないアトリビュート
        # 同様に、例外ではなくログ出力される
        maya_attribute_utils.set_attribute(mesh_name, "non_existent_attr", 1.0, "float")
    ...
    def test_safe_attr_and_json_helpers(self):
        ...
        self.assertEqual(maya_attribute_utils.get_attr_safe(node, "numberAttr", default=0, cast=int), 42)
        self.assertEqual(maya_attribute_utils.get_attr_safe(node, "missingAttr", default="fallback"), "fallback")

        payload = {"0": "笑顔", "1": "wink"}
        self.assertTrue(maya_attribute_utils.write_json_attr(node, "mmd_test_json", payload))
        self.assertEqual(maya_attribute_utils.read_json_attr(node, "mmd_test_json"), payload)

        cmds.setAttr(f"{node}.mmd_test_json", "{broken", type="string")
        self.assertEqual(maya_attribute_utils.read_json_attr(node, "mmd_test_json", default=[]), [])
    ...
    def test_tag_and_connection_helpers(self):
        ...
        self.assertTrue(maya_attribute_utils.mark_bool_tag(source, "mmd_test_tag", True))
        self.assertIn(source, maya_attribute_utils.find_tagged_nodes("mmd_test_tag"))

        self.assertTrue(maya_attribute_utils.connect_if_needed(f"{source}.translateX", f"{dest}.translateX"))
        self.assertTrue(cmds.isConnected(f"{source}.translateX", f"{dest}.translateX"))
        self.assertTrue(maya_attribute_utils.connect_if_needed(f"{source}.translateX", f"{dest}.translateX"))

        self.assertEqual(maya_attribute_utils.disconnect_sources(f"{dest}.translateX"), 1)
    ...
    def test_set_attribute_with_custom_attributes(self):
        ...
        # set_attributeで値を更新
        maya_attribute_utils.set_attribute(test_obj, "intAttr", 100, "long")
        self.assertEqual(cmds.getAttr(test_obj + ".intAttr"), 100)

        maya_attribute_utils.set_attribute(test_obj, "floatAttr", 6.28, "double")
        self.assertAlmostEqual(cmds.getAttr(test_obj + ".floatAttr"), 6.28, places=5)

        maya_attribute_utils.set_attribute(test_obj, "stringAttr", "World", "string")
        self.assertEqual(cmds.getAttr(test_obj + ".stringAttr"), "World")

        maya_attribute_utils.set_attribute(test_obj, "boolAttr", False, "bool")
        self.assertEqual(cmds.getAttr(test_obj + ".boolAttr"), False)

        maya_attribute_utils.set_attribute(test_obj, "vectorAttr", [4.0, 5.0, 6.0], "double3")
    ...
    def test_set_custom_attributes(self):
        ...
        maya_attribute_utils.set_custom_attributes(mesh_name, data)
        ...
        maya_attribute_utils.set_custom_attributes(mesh_name, update_data)
    ...
    def test_repair_fbx_mojibake_string_restores_utf8_as_cp932_text(self):
        ...
        self.assertEqual(maya_attribute_utils.repair_fbx_mojibake_string(mojibake), "センター")
        self.assertEqual(maya_attribute_utils.repair_fbx_mojibake_string("Test Model EN"), "Test Model EN")
    ...
    def test_get_attribute_repairs_mmd_string_mojibake_only_for_mmd_attrs(self):
        ...
        self.assertEqual(maya_attribute_utils.get_attribute(node, "mmd_bone_name"), "センター")
        self.assertEqual(maya_attribute_utils.get_attribute(node, "plain_name"), mojibake)
    ...
    def test_get_custom_attributes(self):
        ...
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "unknown_attribute"), None)

        # Get the attributes
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_bytes"), "PMX")
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_file_version"), 2.0)
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_utf8"), "裙花蕊颜顔")
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_string"), "Test Model EN")
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_bool"), True)
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_int"), 42)
        self.assertAlmostEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_float"), 3.14, places=5)  # type: ignore
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_double3"), (1.0, 2.0, 3.0))
        self.assertEqual(maya_attribute_utils.get_attribute(mesh_name, "mmd_double4"), (1.0, 2.0, 3.0, 4.0))
    ...
    def test_create_ik_handle(self):
        ...
        # IKハンドルを作成
        ik_handle, effector = maya_rig_utils.create_ik_handle(start_joint=joint1, end_joint=joint3, solver="ikRPsolver")
    ...
    def test_create_ik_handle_invalid_joint(self):
        ...
        with self.assertRaises(ValueError) as context:
            maya_rig_utils.create_ik_handle(start_joint="nonexistent_joint", end_joint="another_nonexistent_joint")
    ...
    def test_set_joint_limits(self):
        ...
        maya_rig_utils.set_joint_limits(joint=joint, limit_min=limit_min, limit_max=limit_max, enable_limits=True)
    ...
    def test_matrix_to_euler(self):
        """行列からオイラー角への変換が正しいか"""
        # 単位行列の場合
        identity_matrix = maya_transform_utils.create_matrix_from_axes(x_axis=[1, 0, 0], y_axis=[0, 1, 0], z_axis=[0, 0, 1])

        euler = maya_transform_utils.matrix_to_euler(identity_matrix)
    ...
    def test_get_mmd_model_display_name(self):
        ...
        service = SceneModelService(cmds_module=cmds)
        display_name1 = service.get_model_display_name(model1)
        ...
        display_name2 = service.get_model_display_name(model2)
        self.assertEqual(display_name2, "Hatsune Miku")

        # 名前属性がないモデル
        model3 = cmds.group(empty=True, name="Model3_root")
        display_name3 = service.get_model_display_name(model3)
    ...
    def test_select_objects(self):
        ...
        # 選択をクリア
        result = maya_scene_utils.select_objects(clear=True)
        self.assertTrue(result)
        self.assertEqual(cmds.ls(selection=True), [])

        # 単一オブジェクトを選択
        result = maya_scene_utils.select_objects(cube1)
        self.assertTrue(result)
        self.assertEqual(cmds.ls(selection=True), [cube1])

        # 複数オブジェクトを選択
        result = maya_scene_utils.select_objects([cube1, cube2])
        self.assertTrue(result)
        ...
        # 追加モードで選択
        maya_scene_utils.select_objects(cube1)  # まずcube1を選択
        result = maya_scene_utils.select_objects(cube2, add=True, clear=False, replace=False)
        self.assertTrue(result)
        selected = cmds.ls(selection=True)
        self.assertIn(cube1, selected)
        self.assertIn(cube2, selected)
    ...
    def test_object_exists(self):
        ...
        # 存在しないオブジェクト
        self.assertFalse(maya_scene_utils.object_exists("nonexistent_object"))
    ...
    def test_parent_objects(self):
        ...
        # 単一の子を親付け
        result = maya_scene_utils.parent_objects(child1, parent)
        self.assertEqual(len(result), 1)
        self.assertEqual(cmds.listRelatives(child1, parent=True)[0], parent)

        # 複数の子を親付け
        result = maya_scene_utils.parent_objects([child2], parent)
        self.assertEqual(len(result), 1)
        ...
        result = maya_scene_utils.parent_objects(child3, world=True)
        self.assertEqual(len(result), 1)
        # ワールド空間にある場合、親はNone
    ...
    def test_list_objects(self):
        ...
        # トランスフォームをリスト
        transforms = maya_scene_utils.list_objects(type="transform")
        ...
        # ワイルドカードでフィルター
        test_objects = maya_scene_utils.list_objects(object_filter="*test*")
    ...
    def test_set_attribute_performance(self):
        ...
        for i in range(100):
            maya_attribute_utils.set_attribute(test_obj, "translateX", float(i), "double")
            maya_attribute_utils.set_attribute(test_obj, "rotateY", float(i * 2), "double")
            maya_attribute_utils.set_attribute(test_obj, "scaleZ", float(i * 0.01 + 1), "double")
        ...
        print(f"maya_attribute_utils.set_attribute (API): {api_time:.4f} seconds")
    ...
    def test_create_animation_curves_with_layer_does_not_touch_selection(self):
        ...
        with patch(
            "mmd_tools.core.maya_animation_utils.cmds.select",
    ...
    def test_set_attribute_edge_cases(self):
        ...
        maya_attribute_utils.set_attribute(test_obj, "translateX", -1e10, "double")
        maya_attribute_utils.set_attribute(test_obj, "translateY", 1e10, "double")
        ...
        # Test 2: 0値
        maya_attribute_utils.set_attribute(test_obj, "rotateX", 0.0, "double")
        ...
        # maya_attribute_utils.set_attributeはラジアン値を期待している
        maya_attribute_utils.set_attribute(test_obj2, "rotateY", radians_value, "double")
        ...
        maya_attribute_utils.set_attribute(test_obj, "visibility", False, "bool")
        self.assertEqual(cmds.getAttr(test_obj + ".visibility"), False)
        maya_attribute_utils.set_attribute(test_obj, "visibility", True, "bool")
        self.assertEqual(cmds.getAttr(test_obj + ".visibility"), True)

        # Test 5: カスタムアトリビュートでのエッジケース
        # まずカスタムアトリビュートを作成
        maya_attribute_utils.set_custom_attributes(test_obj, {"stringAttr": "", "floatAttr": 0.0, "intAttr": -999999})

        # 空文字列の設定
        maya_attribute_utils.set_attribute(test_obj, "stringAttr", "", "string")
        self.assertEqual(cmds.getAttr(test_obj + ".stringAttr"), "")

        # 非常に長い文字列
        long_string = "a" * 1000
        maya_attribute_utils.set_attribute(test_obj, "stringAttr", long_string, "string")
        self.assertEqual(cmds.getAttr(test_obj + ".stringAttr"), long_string)

        # 特殊文字を含む文字列
        special_string = "!@#$%^&*()_+-=[]{}|;':\",./<>?"
        maya_attribute_utils.set_attribute(test_obj, "stringAttr", special_string, "string")
        ...
        maya_attribute_utils.set_attribute(test_obj, "translateX", None, "double")
        # 値は変更されないはず

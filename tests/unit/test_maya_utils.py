...
class TestMayaUtils(MayaTestBase):
    ...
    def test_create_ik_handle_invalid_joint(self):
        ...
        with self.assertRaises(ValueError) as context:
            maya_utils.create_ik_handle(
                start_joint="nonexistent_joint",
                end_joint="another_nonexistent_joint"
            )
    ...
    def test_set_joint_limits(self):
        ...
        # 制限が設定されているか確認
        self.assertAlmostEqual(cmds.getAttr(f"{joint}.rotateMinX"), limit_min[0], places=5)
        self.assertAlmostEqual(cmds.getAttr(f"{joint}.rotateMinY"), limit_min[1], places=5)
        self.assertAlmostEqual(cmds.getAttr(f"{joint}.rotateMinZ"), limit_min[2], places=5)
        self.assertAlmostEqual(cmds.getAttr(f"{joint}.rotateMaxX"), limit_max[0], places=5)
        self.assertAlmostEqual(cmds.getAttr(f"{joint}.rotateMaxY"), limit_max[1], places=5)
        self.assertAlmostEqual(cmds.getAttr(f"{joint}.rotateMaxZ"), limit_max[2], places=5)
    ...
    def test_cross_product(self):
        ...
        vec1 = [1, 0, 0]
        vec2 = [0, 1, 0]
        result = maya_utils.cross_product(vec1, vec2)
        ...
        # Y × Z = X
        vec3 = [0, 0, 1]
        result2 = maya_utils.cross_product(vec2, vec3)

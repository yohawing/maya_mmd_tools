...
class TestPmxParser(TestBase):
    ...
    def test_parse_pmx_header_success(self):
        ...
        self.assertIn(header.text_encoding, ['utf-16-le', 'utf-8'], msg=f"不正なテキストエンコーディングです: {header.text_encoding}")
        # 追加UV数が0から4の範囲内であることを確認
        self.assertIn(header.additional_uv, [0, 1, 2, 3, 4], msg=f"追加UV数が不正です: {header.additional_uv}")
        # 各インデックスサイズが1, 2, 4のいずれかであることを確認
        self.assertIn(header.vertex_index_size, [1, 2, 4], msg=f"頂点インデックスサイズが不正です: {header.vertex_index_size}")
        self.assertIn(header.texture_index_size, [1, 2, 4], msg=f"テクスチャインデックスサイズが不正です: {header.texture_index_size}")
        self.assertIn(header.material_index_size, [1, 2, 4], msg=f"材質インデックスサイズが不正です: {header.material_index_size}")
        self.assertIn(header.bone_index_size, [1, 2, 4], msg=f"ボーンインデックスサイズが不正です: {header.bone_index_size}")
        self.assertIn(header.morph_index_size, [1, 2, 4], msg=f"モーフインデックスサイズが不正です: {header.morph_index_size}")
        self.assertIn(header.rigid_body_index_size, [1, 2, 4], msg=f"剛体インデックスサイズが不正です: {header.rigid_body_index_size}")
    ...
    def test_parse_pmx_vertex_success(self):
        ...
        self.assertEqual(len(vertex.position), 3, msg=f"頂点位置の要素数が不正です: {len(vertex.position)}")
        self.assertEqual(len(vertex.normal), 3, msg=f"頂点法線の要素数が不正です: {len(vertex.normal)}")
        self.assertEqual(len(vertex.uv), 2, msg=f"UV座標の要素数が不正です: {len(vertex.uv)}")
        # ウェイト変形方式が0から4のいずれかであることを確認
        self.assertIn(vertex.weight_transform_type, [0, 1, 2, 3, 4], msg=f"不正なウェイト変形方式です: {vertex.weight_transform_type}")
        # エッジ倍率が浮動小数点数であることを確認
        self.assertIsInstance(vertex.edge_magnification, float, msg=f"エッジ倍率がfloatではありません: {type(vertex.edge_magnification)}")
    ...
    def test_parse_pmx_texture_success(self):
        ...
        self.assertIsInstance(self.parsed_data.textures[0], str, msg="テクスチャパスが文字列ではありません")
    ...
    def test_parse_pmx_material_success(self):
        ...
        self.assertIsInstance(material.specular_coefficient, float, msg="Specular係数がfloatではありません")
        ...
        self.assertIsInstance(material.sphere_texture_index, int, msg="スフィアテクスチャインデックスがintではありません")
        ...
        self.assertIsInstance(material.toon_texture_index, int, msg="Toonテクスチャインデックスがintではありません")
    ...
    def test_parse_pmx_morph_success(self):
        ...
        self.assertEqual(len(morph.offsets), morph.offset_count, msg="オフセット数とオフセットのリストの長さが一致しません")
    ...
    def test_parse_pmx_rigid_body_success(self):
        ...
        self.assertIsInstance(rigid_body.related_bone_index, int, msg="関連ボーンインデックスがintではありません")
        self.assertIsInstance(rigid_body.group, int, msg="グループがintではありません")
        self.assertIsInstance(rigid_body.collision_mask, int, msg="非衝突グループフラグがintではありません")
    ...
    def test_parse_pmx_joint_success(self):
        ...
        self.assertIsInstance(joint.rigid_body_a_index, int, msg="関連剛体Aのインデックスがintではありません")
        self.assertIsInstance(joint.rigid_body_b_index, int, msg="関連剛体Bのインデックスがintではありません")

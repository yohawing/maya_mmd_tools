...
class TestInfoPresenter(MayaTestBase):
    ...
    def _create_test_mmd_model(self):
        ...
        cmds.addAttr(root, ln="mmd_model_name_jp", dt="string")
        cmds.addAttr(root, ln="mmd_model_name_en", dt="string")
        cmds.addAttr(root, ln="mmd_comment_jp", dt="string")
        cmds.addAttr(root, ln="mmd_comment_en", dt="string")
        
        # 初期値設定
        cmds.setAttr(f"{root}.mmd_model_name_jp", "テストモデル", type="string")
        cmds.setAttr(f"{root}.mmd_model_name_en", "Test Model", type="string")
        cmds.setAttr(f"{root}.mmd_comment_jp", "テストコメント", type="string")
        cmds.setAttr(f"{root}.mmd_comment_en", "Test Comment", type="string")
    ...
    def _create_mock_view(self):
        ...
        # textChangedシグナルのモック
        for widget in [view.model_name_jp_edit, view.model_name_en_edit,
                      view.comment_jp_edit, view.comment_en_edit]:
    ...
    def test_update_model_info(self):
        ...
        jp_name = cmds.getAttr(f"{self.test_model}.mmd_model_name_jp")
        en_name = cmds.getAttr(f"{self.test_model}.mmd_model_name_en")
        jp_comment = cmds.getAttr(f"{self.test_model}.mmd_comment_jp")
        en_comment = cmds.getAttr(f"{self.test_model}.mmd_comment_en")
    ...
    def test_on_current_model_changed(self):
        ...
        cmds.addAttr(new_model, ln="mmd_model_name_jp", dt="string")
        cmds.addAttr(new_model, ln="mmd_model_name_en", dt="string")
        cmds.addAttr(new_model, ln="mmd_comment_jp", dt="string")
        cmds.addAttr(new_model, ln="mmd_comment_en", dt="string")
        cmds.setAttr(f"{new_model}.mmd_model_name_jp", "新モデル", type="string")
    ...
    def test_signal_connections(self):
        ...
        # テキスト変更シグナルが接続されているか
        for widget in [self.mock_view.model_name_jp_edit, 

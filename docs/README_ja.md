...
1. 疎なControl RigキーのままVMDの回転Bezierを保持したい場合は、`MMD Control Rigを作成`と`VMD回転補間を時間カーブとして保持（実験的）`をONにします。
1. `Import Animation` をクリックします。
1. シーン内の対応するモデルにアニメーションが適用されます。

回転時間カーブmodeでは、回転するControlごとに1本の時間カーブを追加し、XYZ回転をQuaternion短経路で評価します。回転値は編集できますが、回転キーの時刻を追加・移動・削除する場合はXYZ回転カーブと時間カーブを一緒に編集してください。最終姿勢の精度を優先し、疎キー編集が不要な場合は通常の高精度Bakeを使用してください。

...
| 機能 | 状態 | 備考 |
|---|---|---|
| メッシュ | ℹ️ | QDEF/SDEFには非対応です。追加UVレイヤーは開発用PMXエクスポート経路でPMXメタデータとして保持しますが、UVモーフのMayaランタイム評価は未検証です。|
...
| ボーン・スケルトン・リグ（IK・付与・ローカル軸 ） | ℹ️ | 一部対応。複雑なモデルでは既知の問題があり、開発用PMXエクスポート経路では一部のIKメタデータが保持されません。 |
| 表示枠 | ℹ️ | 表示枠の名前・特殊枠・ボーン／モーフ項目を編集できます。 |
| モーフ（頂点・ボーン・マテリアル・グループ・UV） | ℹ️ | 頂点・ボーン・マテリアル・グループは対応済み。UVモーフは開発用のPMXメタデータ再出力用に保持しますが、MayaのUV setを変形せず、未検証です。Flip、Impulseモーフは未対応です。 |
| 物理（剛体・ジョイント） | ℹ️ | 物理の一部編集が未対応となります　|
| ソフトボディ（PMX 2.1） | ⛔ | 非対応 |
| エクスポート | ⛔ | 公開PMX/PMDエクスポートは未対応です。開発用PMX経路ではparse／fresh importの重点検証を行っていますが、PMD要求はpolicy-rejectでfail-closedになります。未対応またはlossyな項目は公開保証の対象外です。 |
...
| エクスポート | ⛔ | 公開VMDエクスポートは未対応です。開発用のMode Cはゲート対象で、Mode Aにはインポート時のraw key／補間provenanceが必要です。provenanceのない経路は拒否またはMode Cへ送られます。 |
...
- **エクスポートは開発用経路のみです** UVモーフはPMXのraw metadataを保持しますが、MayaのUV setには評価されません。一部のIKメタデータは開発用PMXエクスポートでlossyです。PMDエクスポートは拒否され、VMD Mode Aにはインポート時のraw key／補間provenanceが必要です。

...
| マテリアル・テクスチャ | ℹ️ | DX11・OpenGLシェーダーによるMMDトゥーンシェーダーを実装しています。共有トゥーンのインデックスと非共有カスタムトゥーンのパス／インデックスを含むPMX対応マテリアル項目は、インポート・編集・エクスポート・再インポートの重点検証があります。PMDのマテリアルはインポート・表示・編集に対応しますが、PMDエクスポートは拒否されます。Viewport2.0の制約により、MMDシェーダーの再現性が低いです。 |
| Maya向け名前解決 | ✅ | 辞書・またはハッシュにより安全な名前に変換します。またパスのテクスチャを安全なパスに自動解決します。 |
| エッジ／輪郭フラグ | ℹ️ | Viewport2.0の制約があるため、オプションでONにできます。 |
| ボーン・スケルトン・リグ（IK・付与・ローカル軸 ） | ℹ️ | 一部対応。PMX 2.0エクスポートは、IKターゲット・ループ・角度・リンクと制限、および付与・接続先・固定軸／ローカル軸・物理後・外部親の検証済み範囲を再インポートまで保持します。壊れた参照や未解決参照はfail-closedで拒否し、複雑なリグには既知の問題があります。 |
...
| ソフトボディ（PMX 2.1） | ⛔ | 非対応 |
| エクスポート | ℹ️ | Exportワークフローから、上記の検証済み範囲でPMX 2.0を書き出せます。Validationはfail-closedで、拒否時には既存出力を保持します。PMD、SDEF、PMX 2.1固有のFlip／Impulseモーフ、ソフトボディは意図的に非対応です。インポートprovenanceの保持は安定したpolicy-rejectのためであり、機能対応を意味しません。PMX全項目や表示の完全一致は保証しません。 |
...
| カメラアニメーション | ✅ | `mmd_camera` を作成・キー設定し、ライティングは`mmd_light` コントローラを駆動します。VMD Mode CはMaya 2024/2026でカメラ／ライトを密サンプリング検証済みですが、カメラ補間はraw保持せずdense bakeへ正規化します。セルフ影は非対応で、カメラ／ライトのvisual parityは保証しません。 |
...
| コントロールリグ | 🧪 | 準標準ボーン構成に基づいたコントロールリグが自動的に生成されます |
| エクスポート | ℹ️ | ExportワークフローからVMDを書き出せます。Mode Cはモデル・カメラ・ライトをdense bakeし、raw補間は保持しません。Mode Aでraw保持を検証済みなのは、インポート時provenanceが一致する未編集モーションのボーンキーと64-byteボーン補間だけです。モーフ・カメラ・ライト・セルフ影trackのMode A raw保持は検証範囲外です。編集済み・不一致・provenanceなしはfail-closedで拒否するかMode Cを使用します。 |

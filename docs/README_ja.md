...
| 機能 | 状態 | 備考 |
|---|---|---|
| メッシュ | ℹ️ | QDEF/SDEFには非対応です。追加UVレイヤーは開発用PMXエクスポート経路では保持されません。|
| マテリアル・テクスチャ | ℹ️ | DX11・OpenGLシェーダーによるMMDトゥーンシェーダーを実装しています。PMXの対応マテリアル項目は、インポート・編集・エクスポート・再インポートの重点検証があります。PMDのマテリアルはインポート・表示・編集に対応しますが、PMDエクスポートは拒否されます。Viewport2.0の制約により、MMDシェーダーの再現性が低いです。 |
...
- **エクスポートは開発用経路のみです** 追加UVレイヤーと一部のIKメタデータは開発用PMXエクスポートで保持されません。PMDエクスポートは拒否され、VMD Mode Aにはインポート時のraw key／補間provenanceが必要です。

...
3. （任意）アニメーションインポート設定で VMD FPS を設定します（選択肢: 30 または 60、デフォルト 30）。これによりインポート前にMayaシーンの時間単位が変更されます。
4. `Import Animation` をクリックします。
5. シーン内の対応するモデルにアニメーションが適用されます。

注: VMDは整数フレーム番号を格納しますがFPSメタデータを持ちません。フレーム番号は変更せずそのまま使用され（リスケールされません）、VMD FPSの選択に従ってシーンの時間単位のみが設定されます。

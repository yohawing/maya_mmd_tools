...
- [ ] `[test]` **IK move gate に膝方向を追加** — `tests/viewport/gui_ik_probe.py` / mayapy probe で `left_leg_ik` を代表方向へ動かした時、足首距離だけでなく膝 world position または link 制限符号に基づく bend direction を検証する。現行の `enabled=True` / output delta だけでは逆膝を検出できない
- [ ] `[impl]` **Rig VMD live 入力分解を修正** — runtime bake を避けて legacy VMD 直入力に戻すと、Rig VMD mesh は public oracle に FAIL（overall max `10.744267`, mean `1.807621`）。IK 無効でも同じなので IK ではなく、腕/手首系など通常回転/付与入力の空間不一致。mmd-anim/runtime final local と legacy direct VMD key が大きく違うため、Rig mode は runtime final を `mmdAppend.base*` / IK input / 通常 joint.rotate へ分解して live rig を保つ経路が必要。report: `build/reports/after_append_translate_gate_public_v020_addiction_rig_frames_0_5.md`

...
| VPD | ✅ | Export the current character pose from the Export tab and apply VPD by drag-and-drop. |
...
| Export | 🧪 | Exports the current character pose as VPD, or the current character and an explicitly selected camera and/or light over the selected timeline as VMD (Bake export only). Character and scene tracks are exported separately; self-shadow export is unsupported. |

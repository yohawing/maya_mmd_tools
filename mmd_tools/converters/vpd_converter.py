...
from mmd_tools.core.maya_utils import get_attribute, set_attribute_value_api
...
class VpdConverter:
    ...
    def __init__(self):
        ...
        self._initialize_bone_name_mapping()
    
    def _initialize_bone_name_mapping(self):
        """標準的なボーン名マッピングを初期化"""
    ...
    def _find_maya_joint(self, mmd_bone_name, joints, namespace=None):
        ...
        if mmd_bone_name in self.bone_name_mapping:
            possible_names = self.bone_name_mapping[mmd_bone_name]
            for joint in joints:
                joint_name = joint.split(":")[-1] if ":" in joint else joint
    ...
    def _apply_joint_orient_correction(self, rotation, joint_orient):
        ...
        return [
            rotation[0] - joint_orient[0],
            rotation[1] - joint_orient[1],
            rotation[2] - joint_orient[2]
        ]
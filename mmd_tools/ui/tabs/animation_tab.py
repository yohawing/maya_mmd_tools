...
class AnimationTab(BaseTab):
    ...
    def __init__(self, parent=None):
        ...
        from ..widgets.morph_editor_widgets import MorphWeightSpinBox

        self.morph_filter = QLineEdit()
        self.morph_filter.setPlaceholderText("Filter morphs")
        morph_outer.addWidget(self.morph_filter)
        batch_layout = QHBoxLayout()
        self.morph_batch_status = QLabel("Select morphs (Ctrl / Shift)")
        batch_layout.addWidget(self.morph_batch_status, 1)
        self.morph_batch_weight = MorphWeightSpinBox()
        self.morph_batch_weight.setEnabled(False)
        batch_layout.addWidget(self.morph_batch_weight)
        self.morph_batch_apply = QPushButton("Apply")
        self.morph_batch_apply.setEnabled(False)
        batch_layout.addWidget(self.morph_batch_apply)
        morph_outer.addLayout(batch_layout)

...
def _vmd_descriptor(path: Path) -> dict[str, Any]:
    ...
    for section, frames, name_field in sections:
        ...
        for frame in frames:
            frame_numbers.append(_frame_number(frame))
            if name_field:
                ...
                for name, _state in frame.ik_states:
                    active_tracks.add((section, _normalized_name(name)))
            else:
                active_tracks.add((section, "<single>"))
...
def _pair_score(vmd: Mapping[str, Any], pmx: Mapping[str, Any]) -> dict[str, float]:
    """Calculate compatibility metrics used to exclude mismatched pairs."""
    ...
    has_bones = bool(vmd.get("_bone_names"))
    combined = bone_ratio if has_bones else morph_ratio
    return {
...
def _select_pmx(
    ...
    count: int,
    vmd_descriptors: Iterable[Mapping[str, Any]] = (),
    min_bone_match: float = 0.25,
) -> list[dict[str, Any]]:
    """Select compatible small/feature-rich/large PMX representatives."""
    ...
    usable = [
        ...
        if item["metrics"]["bones"] > 0 and item["metrics"]["vertices"] > 0
    ]
    if not usable:
        raise ValueError("no PMX with parsed bones was found")
    motion_descriptors = [
        item
        for item in vmd_descriptors
        if item["metrics"]["bone_frames"] >= 20 and len(item.get("_bone_names", ())) >= 2
    ]

    def compatibility_key(item: Mapping[str, Any]) -> tuple[float, ...]:
        ratios = sorted(
            (
                _match_ratio(motion.get("_bone_names", ()), item.get("_bone_names", ()))
                for motion in motion_descriptors
            ),
            reverse=True,
        )
        compatible_count = sum(ratio >= min_bone_match for ratio in ratios)
        return (
            compatible_count,
            ratios[0] if ratios else 0.0,
            sum(ratios[:10]),
    ...
    if len(selected) < count:
        largest = max(
            (item for item in usable if item not in selected),
            key=lambda item: (compatibility_key(item), int(item["metrics"]["vertices"])),
            default=None,
        )
...
def _select_motion_cases(
    ...
    *,
    ...
    dense_candidates = [
        ...
        and item[0]["metrics"]["bone_frames"] >= 1_000
    ]
    sparse_candidates = [
        item
        for item in candidates
        if item[0]["metrics"]["density"] <= sparse_density
        and item[0]["metrics"]["bone_frames"] >= 20
        and item[0]["metrics"]["interpolation_variant_count"] >= 2
    ]
    if len(dense_candidates) < dense_count:
        raise ValueError(
            f"only {len(dense_candidates)} compatible dense VMD candidates; {dense_count} required"
        )
    if len(sparse_candidates) < sparse_count:
        raise ValueError(
            f"only {len(sparse_candidates)} compatible sparse VMD candidates; {sparse_count} required"
        )
...
def build_manifest(
    ...
    *,
    ...
    selected_pmx = _select_pmx(
        pmx_descriptors,
        pmx_count,
        vmd_descriptors,
        min_bone_match=min_bone_match,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not 0.0 <= args.min_bone_match <= 1.0:
        raise SystemExit("min bone match must be between 0 and 1")
    ...
    manifest = build_manifest(
        ...
        min_bone_match=args.min_bone_match,
    )
